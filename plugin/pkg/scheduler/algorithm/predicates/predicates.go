...
func PodFitsResources(pod *api.Pod, meta interface{}, nodeInfo *schedulercache.NodeInfo) (bool, []algorithm.PredicateFailureReason, error) {
	...
	sumBufferedGPURequestMem := nodeInfo.SumBufferedGPURequestMemory()
	...
	for _, mem := range memoryOfEachNvidiaGPU {
		// We do not know which GPU will be assigned
		// So we make an under-estimation of available memory for each GPU
		memInt64 := (*mem).Value() - sumBufferedGPURequestMem
